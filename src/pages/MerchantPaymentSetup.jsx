...
import { Building2, CreditCard, Plus, Search, WalletCards } from 'lucide-react'
...
export default function MerchantPaymentSetup() {
  ...
  return <div className="flex h-full flex-col">
    <Topbar title="إعداد دفع التجار" subtitle="طرق الدفع والحسابات لكل تاجر رئيسي وتاجر فرعي" onRefresh={setup.refresh} isFetching={setup.isFetching} actions={<button onClick={() => setAddOpen(true)} className="flex items-center gap-2 rounded-lg bg-gold px-3 py-2 text-sm font-bold text-bg hover:opacity-90"><Plus size={16} /> إضافة إعداد</button>} />
    <div className="flex-1 space-y-5 overflow-y-auto p-4 md:p-6">
      ...
      <div className="overflow-x-auto rounded-2xl border border-border bg-card">
        <table className="w-full min-w-[950px] text-sm"><thead className="bg-surface text-xs text-muted"><tr><th className="px-4 py-3 text-right">التاجر الرئيسي</th><th className="px-4 py-3 text-right">التاجر الفرعي</th><th className="px-4 py-3 text-right">طريقة الدفع</th><th className="px-4 py-3 text-right">الحساب</th><th className="px-4 py-3 text-right">المزود / الاسم</th><th className="px-4 py-3 text-right">الحالة</th><th className="px-4 py-3 text-right">إجراء</th></tr></thead><tbody>{rows.map((row) => <tr key={row.id} className="border-t border-border"><td className="px-4 py-3 font-semibold">{row.master_merchant}</td><td className="px-4 py-3">{row.sub_merchant}</td><td className="px-4 py-3 font-medium">{row.payment_method}</td><td className="px-4 py-3 font-mono text-gold">{row.account_number}</td><td className="px-4 py-3 text-muted">{row.provider || '—'}{row.account_name ? ` · ${row.account_name}` : ''}</td><td className="px-4 py-3"><span className={`rounded-full px-2 py-1 text-xs font-bold ${row.active ? 'bg-success/15 text-success' : 'bg-danger/15 text-danger'}`}>{row.active ? 'نشط' : 'موقوف'}</span></td><td className="px-4 py-3"><button onClick={() => toggle(row)} className="rounded-lg border border-border px-3 py-1.5 text-xs text-text hover:border-gold">{row.active ? 'إيقاف' : 'تفعيل'}</button></td></tr>)}{!rows.length && <tr><td colSpan="7" className="px-4 py-12 text-center text-sm text-muted">لا توجد إعدادات مطابقة للفلاتر</td></tr>}</tbody></table>
      </div>
