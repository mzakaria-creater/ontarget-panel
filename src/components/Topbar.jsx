  ...
  return (
    <div className="flex items-center justify-between border-b border-border bg-surface/60 px-6 py-4">
      ...
      <div className="flex items-center gap-2">
        ...
        <button onClick={toggleLanguage} className="rounded-lg border border-border bg-card px-2.5 py-2 text-xs font-bold text-muted hover:border-gold/50 hover:text-gold">{language === 'ar' ? 'EN' : 'عربي'}</button>
        <button onClick={toggleTheme} className="rounded-lg border border-border bg-card p-2 text-base leading-none text-muted hover:border-gold/50 hover:text-gold" title={theme === 'dark' ? 'Switch to light theme' : 'Switch to dark theme'} aria-label={theme === 'dark' ? 'Switch to light theme' : 'Switch to dark theme'}>{theme === 'dark' ? '☀️' : '🌙'}</button>
        <div className="relative">
          <button onClick={() => setSmsOpen((open) => !open)} className="relative rounded-lg border border-border bg-card p-2 text-muted hover:border-gold/50 hover:text-gold" title="SMS notifications"><span className="text-base leading-none">📨</span>{smsNotifications.length > 0 && <span className="absolute -right-1 -top-1 flex h-4 min-w-4 items-center justify-center rounded-full bg-danger px-1 text-[10px] font-bold text-white">{smsNotifications.length > 9 ? '9+' : smsNotifications.length}</span>}</button>
        ...
        {onRefresh && <button onClick={onRefresh} className="flex items-center gap-2 rounded-lg border border-border bg-card px-3 py-2 text-sm font-medium text-text hover:border-gold/50 hover:text-gold" title="تحديث (R)"><span className={isFetching ? 'animate-spin' : ''}>🔄</span>تحديث</button>}
      </div>
    </div>
  )
}
